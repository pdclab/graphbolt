...
template <typename E, typename F> struct in_imap {
  using T = E;
  F f;
  size_t s, e;
...
template <class E, class F> in_imap<E, F> make_in_imap(size_t n, F f) {
  return in_imap<E, F>(n, f);
}

template <typename E, typename F> struct out_imap {
  using T = E;
  F f;
  size_t s, e;
...
// used so second template argument can be inferred
template <class E, class F> out_imap<E, F> make_out_imap(size_t n, F f) {
  return out_imap<E, F>(n, f);
}

template <typename Iterator> struct iter_imap {
...
template <typename E> struct array_imap {
  using T = E;
  E *s, *e;
  bool alloc;
...
// used so template argument can be inferred
template <class E> array_imap<E> make_array_imap(E *A, size_t n) {
